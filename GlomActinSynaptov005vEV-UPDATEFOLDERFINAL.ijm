...
//    Modifications to ROIs and measurements by Elisavet Vasilopoulou University of Kent 2021.
  ...
  saveSettings();
// This version will work on a folder at a time. 
...
//Get the folder of images, and a save location
	dir1 = getDirectory("Where are your images saved?..");
	list = getFileList(dir1); //make a list of the filenames
	savelocation = getDirectory("Choose a Directory to save the ROIs");

// work in th ebackground
	setBatchMode("hide");

// get the time to time the macro
	start = getTime();
	
// work through the folder a file at a time
...
// save the image
	saveAs("Tif", savelocation+name+"-Processed");
	run("Close");
	
// Make a tidy results table
	// Get the datya from the results table
...
// save the raw results	& close them
	selectWindow("Results");
	saveAs("Results", savelocation+name+"-RawResults.csv");
	run("Close");
...
//Save the compiled table
	selectWindow(table);
	saveAs("Results", savelocation+"AllResults.csv");
	

// Save the ROIs
	roiManager("deselect");
	roiManager("save selected", savelocation+"AllROIs.zip"); // save the ROIs.

// get the end time to time the macro
		end = getTime();
		t = (end-start)/1000;
...
// All done messsage
	...
	Dialog.addMessage("            ******** All done ********\n \n"+i+" images Analysed in "+t+" seconds\n \n    Any problems or bugs contact\nDale (email: d.moulding-at-ucl.ac.uk)\n \nPlease acknowledge use of this macro :)");
	Dialog.show();
	exit();
